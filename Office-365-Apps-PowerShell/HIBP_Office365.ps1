    ...
    $emails | ForEach-Object {
        ...
        Catch {
            if($error[0].Exception.response.StatusCode -match "NotFound"){
                ...
                Write-Host "Cannot retrieve results due to rate limiting or suspect IP"
            }
        }
        if ($BreachResult) {
...
If ($Breaches -gt 0) {
    $Report | Export-CSV C:\Temp\HIBP_Office365_Users.csv -NoTypeInformation
    Write-Host "Total breaches found: " $Breaches " You can find a report in c:\temp\Breaches.csv" }
Else 
  { Write-Host "Hurray - currenlty there are no breaches found ob your Office 365 users" }